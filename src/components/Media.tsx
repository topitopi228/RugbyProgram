...
const Media = () => {
  ...
  const [externalLinks] = useState<ExternalLink[]>([
    {
      ...
      description: { UA: 'Оновлення з тренувального збору', EN: 'Training camp update', HUN: 'Edzőtábori frissítés' },
    ...
    },
    {
      ...
      description: { UA: 'БФ "Міла" та Олексій Юренко допомогли юним регбістам U16', EN: 'BF "Mila" and Oleksiy Yurenko helped young U16 rugby players', HUN: 'A "Mila" Alapítvány és Olekszij Jurenko segítette a fiatal U16-os rögbistákat' },
      image: '/post2.webp',
    },
    {
      id: 3,
      ...
      description: { UA: 'Завдяки підтримці БФ "Міла" та Олексія Юренка', EN: 'Thanks to the support of BF "Mila" and Oleksiy Yurenko', HUN: 'A "Mila" Alapítvány és Olekszij Jurenko támogatásának köszönhetően' },
      image: '/post3.webp',
    },
    {
      id: 4,
      ...
      description: { UA: 'Збори збірної України з регбі U16', EN: 'Training camp of Ukraine U16 rugby team', HUN: 'Az ukrán U16-os rögbi válogatott edzőtábora' },
      image: '/post4.webp',
    },
    {
      id: 5,
    ...
    },
    {
      ...
      description: { UA: 'Новини про підготовку збірної U16', EN: 'News about U16 team preparation', HUN: 'Hírek az U16-os válogatott felkészüléséről' },
      image: '/post6.webp',
    },
    {
      id: 7,
      ...
      description: { UA: 'Огляд тренувального процесу', EN: 'Training process overview', HUN: 'Az edzésfolyamat áttekintése' },
    ...
    const loadMedia = async () => {
      try {
        const items: MediaItem[] = [
          { id: 1, type: 'video', url: '/media12.webm', poster: '/media12_poster.jpg', title: { UA: 'Тренування U16', EN: 'U16 Training', HUN: 'U16 Mérkőzés' }, category: 'matches' },
          { id: 3, type: 'video', url: '/media15.webm', poster: '/media15_poster.jpg', title: { UA: 'Командні тренування', EN: 'Team Training', HUN: 'Csapat edzés' }, category: 'training' },
          { id: 4, type: 'video', url: '/media16.webm', poster: '/media16_poster.jpg', title: { UA: 'Турнірні ігри', EN: 'Tournament Games', HUN: 'Torna mérkőzések' }, category: 'matches' },
          { id: 5, type: 'video', url: '/media17.webm', poster: '/media17_poster.jpg', title: { UA: 'Майстер-клас з гри в захисті', EN: 'Defense Masterclass', HUN: 'Védekezési mesterfokon' }, category: 'training' },
          { id: 6, type: 'video', url: '/media18.webm', poster: '/media18_poster.jpg', title: { UA: 'Атакувальні комбінації', EN: 'Attacking Combinations', HUN: 'Támadó kombinációk' }, category: 'training' },
          { id: 7, type: 'video', url: '/media19.webm', poster: '/media19_poster.jpg', title: { UA: 'Перемога в турнірі', EN: 'Win in the tournament', HUN: 'Bedobások és rúgások' }, category: 'matches' },
          { id: 8, type: 'video', url: '/media20.webm', poster: '/media20_poster.jpg', title: { UA: 'Швидкісні тренування', EN: 'Speed Training', HUN: 'Gyorsasági edzés' }, category: 'training' },
          { id: 9, type: 'video', url: '/media21.webm', poster: '/media21_poster.jpg', title: { UA: 'Тренувальний збір', EN: 'Training Camp', HUN: 'Edzőtábor' }, category: 'training' },
          { id: 11, type: 'video', url: '/media2.webm', poster: '/media2_poster.jpg', title: { UA: 'Фінальний матч', EN: 'Final Match', HUN: 'Döntő mérkőzés' }, category: 'matches' },
          { id: 12, type: 'video', url: '/media3.webm', poster: '/media3_poster.jpg', title: { UA: 'Розминка', EN: 'Final Match', HUN: 'Döntő mérkőzés' }, category: 'training' },
          { id: 13, type: 'video', url: '/media6.webm', poster: '/media6_poster.jpg', title: { UA: 'Тренувальний матч', EN: 'Training Match', HUN: 'Döntő mérkőzés' }, category: 'matches' },
          { id: 14, type: 'video', url: '/media7.webm', poster: '/media7_poster.jpg', title: { UA: 'Тренування', EN: 'Training', HUN: 'Döntő mérkőzés' }, category: 'matches' },
        ];
        setMediaItems(items);
      } catch (error) {
  ...
  const goToNext = () => {
    ...
    const newIndex = mediaItems.findIndex(item => item.id === nextItemId);
  ...
  const goToPrevious = () => {
    ...
    const newIndex = mediaItems.findIndex(item => item.id === prevItemId);
    setCurrentIndex(newIndex);
    setIsPlaying(true);
  };
  ...
  const selectMedia = (index: number) => {
    ...
    const newIndex = mediaItems.findIndex(item => item.id === selectedItemId);
    setCurrentIndex(newIndex);
    setIsPlaying(true);
  };
      ...
      {externalLinks.map((link) => (
        ...
        >
          <div className="flex items-center mb-4">
            <div className={`p-3 rounded-lg ${link.type === 'instagram' ? 'bg-pink-600/20 text-pink-400' : link.type === 'youtube' ? 'bg-red-600/20 text-red-400' : link.type === 'facebook' ? 'bg-blue-600/20 text-blue-400' : 'bg-blue-600/20 text-blue-400'}`}>
          ...
          {link.description && <p className="text-gray-300 text-sm mt-2 flex-grow">{link.description[language]}</p>}
          {link.image && <img src={link.image} alt={link.title[language]} className="w-full h-40 object-cover mb-4" />}
          <div className="mt-4 flex items-center text-yellow-400 text-sm font-medium">
            {language === 'UA' ? 'Відкрити' : language === 'EN' ? 'Open' : 'Megnyitás'} <FaExternalLinkAlt className="w-3 h-3 ml-2" />
          </div>
        </motion.a>
      ))}
    </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="relative pt-[56.25%] bg-black">
                ...
                >
                  {currentItem.type === 'video' ? (
                    <div className="absolute inset-0 group">
                      ...
                      {!isPlaying && (
                        <div className="absolute inset-0 flex items-center justify-center cursor-pointer bg-black/30" onClick={togglePlay}>
                          <motion.div className="w-20 h-20 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center group-hover:scale-110 transition-transform" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <img src={currentItem.url} alt={currentItem.title[language]} className="max-h-full max-w-full object-contain" />
                    </div>
                  )}
                </motion.div>
              </AnimatePresence>
            ...
            {filteredItems.length > 1 && (
              <>
                <button onClick={goToPrevious} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 text-white p-3 rounded-full transition-all z-10 backdrop-blur-sm" aria-label={language === 'UA' ? 'Попереднє' : language === 'EN' ? 'Previous' : 'Előző'}>
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
                  </svg>
                </button>
                <button onClick={goToNext} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 text-white p-3 rounded-full transition-all z-10 backdrop-blur-sm" aria-label={language === 'UA' ? 'Наступне' : language === 'EN' ? 'Next' : 'Következő'}>
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm text-gray-400">{currentIndex + 1} / {filteredItems.length}</span>
                  </div>
                  <h2 className="text-xl font-bold text-white">{currentItem.title[language]}</h2>
                </div>
                <div className="flex items-center gap-3">
                  <button onClick={togglePlay} className="p-2 rounded-full bg-gray-700 hover:bg-gray-600 transition-colors" aria-label={isPlaying ? 'Pause' : 'Play'}>
                    {isPlaying ? (
        ...
        {activeCategory === 'other' ? (
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" variants={container} initial="hidden" animate="show">
                ...
                <div className="relative pt-[56.25%] bg-gray-900">
                  {item.type === 'video' ? (
                    <>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/10 transition-colors">
                        <div className="w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <img src={item.url} alt={item.title[language]} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  )}
                  <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400">{index + 1} / {filteredItems.length}</span>
                    </div>
